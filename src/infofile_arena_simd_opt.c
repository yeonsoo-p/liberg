...
static void ensure_capacity_opt(InfoFileArenaSimdOpt *info) {
    if (info->count >= info->capacity) {
        info->capacity *= 2;
        info->entries = realloc(info->entries, info->capacity * sizeof(InfoFileEntryArenaSimdOpt));
    }
}
...
int infofile_arena_simd_opt_parse_file(const char *filename, InfoFileArenaSimdOpt *info) {
    ...
    int result = infofile_arena_simd_opt_parse_string(buffer, bytes_read, info);
    free(buffer);
