...
int main(void) {
    ...
    printf(" [OK] Arena initialized with 1024 bytes\n\n");
    ...
    memset(ptr1, 'A', 100);
    printf(" [OK] Allocated 100 bytes\n");
    printf(" [OK] Arena usage: %zu / %zu bytes\n\n", arena_get_used(&arena), arena_get_capacity(&arena));
    ...
    printf(" [OK] Three allocations totaling 450 bytes\n");
    printf(" [OK] Arena usage: %zu / %zu bytes\n\n", arena_get_used(&arena), arena_get_capacity(&arena));
    ...
    printf(" [OK] Partial string copied: \"%s\"\n", partial);
    printf(" [OK] Arena usage: %zu / %zu bytes\n\n", arena_get_used(&arena), arena_get_capacity(&arena));
    ...
    printf(" [OK] Arena grew from %zu to %zu bytes\n", old_capacity, arena_get_capacity(&arena));
    printf(" [OK] Large allocation successful\n\n");

    /* Test 7: Verify data integrity after growth */
    printf("Test 7: Data integrity after growth...\n");
    assert(strcmp(dup_str, test_str) == 0);
    assert(strcmp(partial, "This is") == 0);
    printf(" [OK] Previously allocated strings still valid\n");
    printf(" [OK] Data safe with chunk-based arena (no pointer invalidation!)\n\n");
    ...
    printf(" [OK] Arena reset (used = 0, capacity = %zu)\n", arena_get_capacity(&arena));
    printf(" [OK] Memory not freed, ready for reuse\n\n");
    ...
    printf(" [OK] Arena successfully reused after reset\n");
    printf(" [OK] New string: \"%s\"\n\n", new_str);
    ...
    printf(" [OK] Used: %zu bytes\n", used);
    printf(" [OK] Capacity: %zu bytes\n", capacity);
    printf(" [OK] Utilization: %.1f%%\n\n", (used * 100.0) / capacity);

    /* Cleanup */
    arena_free(&arena);
    printf("Test 11: Cleanup...\n");
    printf(" [OK] Arena freed\n\n");
