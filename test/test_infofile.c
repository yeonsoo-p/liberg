...
void test_set_and_get()
{
    ...
    // Add new entries
    infofile_set(&info, "Test.Key1", "Value1");
    infofile_set(&info, "Test.Key2", "Value2");

    const char *val1 = infofile_get(&info, "Test.Key1");
    assert(val1 != NULL);
    assert(strcmp(val1, "Value1") == 0);

    // Update existing entry
    infofile_set(&info, "Test.Key1", "UpdatedValue");
    val1 = infofile_get(&info, "Test.Key1");
...
void test_write_string()
{
    ...
    infofile_set(&info, "Simple.Key", "SimpleValue");
    infofile_set(&info, "Multi.Line", "Line1\nLine2\nLine3");

    char *output = infofile_write_string(&info);
    assert(output != NULL);
...
void test_real_file(const char *filename)
{
    if (!filename)
    {
        ...
        printf("⚠ No file specified (use ./test_infofile <file> to test with real file)\n");
        return;
    }
    ...
    if (result == 0)
    {
        ...
        const char *testrun = infofile_get(&info, "Testrun");
        if (testrun)
        {
            printf("  Testrun = %s\n", testrun);
        }
